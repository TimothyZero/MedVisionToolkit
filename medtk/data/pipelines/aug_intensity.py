...
class Normalize(OperationStage):
    ...
    def __init__(self, mean, std, clip=True, backward_apply_to=('img',)):
        super().__init__()
        self.mean = mean
        self.std = std
        self.clip = clip
    ...
    def apply_to_img(self, result):
        ...
        if self.isForwarding:
            ...
            mean = np.array(mean, dtype=np.float32)[expand]
            std = np.array(std, dtype=np.float32)[expand]
            result['img'] = (result['img'] - mean) / std
            if self.clip:
...
class RandomCutout(AugmentationStage):
    ...
    def apply_to_img(self, result: dict):
        ...
        for hole in self.params:
            ...
            mean_val = self.fusion_fun(result['img'][slices], axis=tuple(range(1, self.dim + 1)))
            result['img'][slices] = mean_val.reshape(-1, *(1,) * self.dim)
            mask[slices] = 1.0
