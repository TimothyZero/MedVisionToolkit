...
from ...ops import cropBBoxes, padBBoxes, nmsNd_numpy
...
class Patches(OperationStage):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += '(patch_size={}, overlap={}, fusion_mode={}, backward_apply_to={})'.format(
            self.patch_size, self.overlap, self.fusion_mode, self.backward_apply_to)
        return repr_str
    ...
    def apply_to_img(self, result):
        for key in result.get('img_fields', []):
            if self.isForwarding:
                ...
                if key not in self.backward_apply_to:
                    return
    ...
    def apply_to_seg(self, result):
        if self.isForwarding:
            ...
            if 'seg' not in self.backward_apply_to:
                return
            for key in result.get('seg_fields', []):
    ...
    def apply_to_det(self, result):
        if self.isForwarding:
            ...
            if 'det' not in self.backward_apply_to:
                return
            for key in result.get('det_fields', []):  # gt_bboxes
                ...
                if dets.shape[1] - 2 * self.dim == 2:
                    score_col = -1
                elif dets.shape[1] - 2 * self.dim == 3:  # last is anchor id
                    score_col = -2
                else:
                    raise NotImplementedError
                if len(dets):
                    k, _ = nmsNd_numpy(dets[:, [*range(self.dim * 2), score_col]], 0.7 ** self.dim)
                    dets = dets[k]
                    # print(dets)
                result[key] = dets
