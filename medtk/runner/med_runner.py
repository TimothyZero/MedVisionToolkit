...
from ..data.parallel.dist_utils import get_dist_info
...
class MedRunner(object):
    ...
    def __init__(self,
                 ...
                 logger,
                 timestamp,
                 cuda_batch_augmentation=None,
                 visual_iter=10,
                 valid_visual_iter=10):
        ...
        self.logger = logger
        self.cuda_batch_augmentation = cuda_batch_augmentation
        self.visual_iter = visual_iter
        self.valid_visual_iter = valid_visual_iter
    ...
    def online_cuda_batch_augmentation(self, data_batch):
        from medtk.data.loader.build_loader import collate

        for transform in self.cuda_batch_augmentation:
            data_batch = transform(data_batch)

        collated_data_batch = collate(data_batch)
        return collated_data_batch
    ...
    def train(self, data_loader, **kwargs):
        ...
        tic = time.time()
        for i, data_batch in enumerate(self.data_loader):  # data batch still on cpu
            if self.cuda_batch_augmentation is not None:
                data_batch = self.online_cuda_batch_augmentation(data_batch)

            if i <= 10:
                if i == 0:
                    print_fun = self.logger.info
                else:
                    print_fun = self.logger.debug
                print_fun(f'Loader init first epoch cost: {time.time() - tic}s')
                for _batch in range(len(data_batch['img_meta'])):
                    try:
                        print_fun(f"time cost: [{data_batch['img_meta'][_batch]['filename']}] => {data_batch['img_meta'][_batch]['time']}")

                        # print_fun(f"time cost: [{data_batch['img_meta'][_batch]['filename']}] => {data_batch['img'][_batch].sum()}")
                        # print_fun(f"time cost: [{data_batch['img_meta'][_batch]['filename']}] => {data_batch['gt_det'][_batch]}")
                        # for k in data_batch['img_meta'][_batch].keys():
                        #     if '_params' in k:
                        #         if isinstance(data_batch['img_meta'][_batch][k], torch.Tensor):
                        #             print_fun(f"{k}: {data_batch['img_meta'][_batch][k].shape}")
                        #         elif isinstance(data_batch['img_meta'][_batch][k], np.ndarray):
                        #             print_fun(f"{k}: {data_batch['img_meta'][_batch][k].shape}")
                        #         else:
                        #             print_fun(f"{k}: {data_batch['img_meta'][_batch][k]}")
                    except:
                        pass
            # else:
            #     raise
            ...
            with autocast():
                # DP or DDP scatter and cuda, then forward, then backward
        ...
        self.logger.info(f'Training epoch-{self._epoch} cost: {time.time() - tic}s')
    ...
    def run(self, data_loaders, workflow, max_epochs, **kwargs):
        ...
        # resume while batch size changed
        if self._iter != self._epoch * len(data_loaders[0]):
            self.logger.info(f'Reset iter from {self._iter} to {self._epoch * len(data_loaders[0])}')
            self._iter = self._epoch * len(data_loaders[0])
