...
class MedDataParallel(DataParallel):
    ...
    #     SUM = torch.sum(torch.tensor(num_samples))
...
if __name__ == "__main__":
    """
    len(output_of_gpus) == num_gpus
    output = network output => 
        ({}, Tensor, ...)
    """
    output_of_gpus = [
        [{'loss': torch.tensor(1.).cuda() * 0.1}, torch.ones((1, 4)).cuda() * 1],
        [{'loss': torch.tensor(1.).cuda() * 0.2}, torch.ones((1, 4)).cuda() * 2],
    ]
    o = gather(output_of_gpus, 'cuda')
    print(o)