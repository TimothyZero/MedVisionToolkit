...
class AnchorFreeHead(BaseDenseHead):
    def __init__(self,
                 ...
                 norm_cfg: dict = None,
                 act_cfg: dict = None):
        super(AnchorFreeHead, self).__init__(conv_cfg, norm_cfg, act_cfg)
    ...
    def _bboxes_single_image(self, cls_out_flat, reg_out_flat, concat_level_anchors, concat_valid):
        ...
        for i in range(cls_out_flat.shape[1]):  # multi classes
            ...
            scores = scores[order[:nms_pre]]
            bboxes = bboxes[order[:nms_pre]]
            labels = labels[order[:nms_pre]]
    ...
    def forward_train(self,
        ...
        self.try_to_info('start anchor free')
        ...
        if self.has_proposal():
            batch_bboxes = self.get_proposals(cls_out_mat, reg_out_mat, anchor_mat, valid_mat)
        else:
            batch_bboxes = None
    ...
    def forward_valid(self,
        ...
        if self.has_proposal():
            ...
            batch_bboxes = self.get_bboxes(cls_out_mat, reg_out_mat, anchor_mat, valid_mat)
    ...
    def forward_infer(self, feats: List[torch.Tensor]):
        ...
        if self.has_proposal():
            proposals = self.get_proposals(cls_out_mat, reg_out_mat, anchor_mat, valid_mat)
            return proposals, (mlvl_cls_out, mlvl_reg_out)
        else:
            batch_bboxes = self.get_bboxes(cls_out_mat, reg_out_mat, anchor_mat, valid_mat)
            return batch_bboxes, (mlvl_cls_out, mlvl_reg_out)
...
if __name__ == "__main__":
    ...
    head = AnchorFreeHead(
        ...
        num_classes=1,
        level_first=True,
        anchor_generator=PointGenerator(
            dim=DIM,
            strides=[2, 4],
            invalid_margin=4,
        ),
        bbox_coder=DistBBoxCoder(
            dim=DIM,
            target_stds=[1., 1., 1., 1.]),
        assigner=DistAssigner(
            pos_dist_thr=0.8,
            max_pos_dist=0.9,
