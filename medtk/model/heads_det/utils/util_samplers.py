...
class BaseSampler(metaclass=ABCMeta):
    ...
    def sample(self,
        ...
        if self.num > 0:
            ...
            num_expected_pos = len(assigned_labels)
...
class HNEMSampler(BaseSampler):
    ...
    def _sample_pos(self,
        ...
        if pos_indices.numel() != 0:
            pos_indices = pos_indices.squeeze(1)
        if len(pos_indices) <= num_expected:
            return pos_indices
        else:
            # print('hard mining pos')
            # print(pos_indices)
...
if __name__ == '__main__':
    ...
    from medtk.model.heads_det.utils.util_assigners import *
    ...
    assigner = MaxIoUAssigner(pos_iou_thr=0.7, neg_iou_thr=0.5, min_pos_iou=0.2, match_low_quality=True)
    assigned_labels, assigned_bboxes = assigner.assign(anchors, gt_bboxes=gt_bboxes, gt_labels=gt_labels)
    print(iou)
    print(assigned_labels)

    sampler = HNEMSampler(pos_fraction=0.5)
    pos_indices, neg_indices, bboxes, sampled_assigned_labels, sampled_assigned_bboxes = sampler.sample(
        bboxes=anchors,
        assigned_labels=assigned_labels,
        assigned_bboxes=assigned_bboxes,
        gt_labels=gt_labels,
        gt_bboxes=gt_bboxes,
        weight=1 - iou.max(dim=1)[0],
    )
    print(pos_indices, neg_indices)
    print(sampled_assigned_labels, sampled_assigned_bboxes)