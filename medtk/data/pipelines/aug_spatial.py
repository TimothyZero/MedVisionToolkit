...
from ...ops import cropBBoxes, clipBBoxes, objs2bboxes, bboxes2objs
from .aug_base import OperationStage, AugmentationStage

# TODO: add robust shift and scale
...
class Resize(OperationStage):
    ...
    def apply_to_img(self, result):
        for key in result.get('img_fields', []):
            img = result.pop(key)  # .astype(np.float32)
...
class Pad(OperationStage):
    ...
    def __init__(self, size, mode='constant', val=0, center=True, backward_apply_to=('img', 'seg', 'det')):
        super().__init__()
        self.size = size
        self.mode = mode
        self.val = val
        self.center = center
        self.backward_apply_to = backward_apply_to
    ...
    def apply_to_img(self, result):
        for key in result.get('img_fields', []):
            ...
            if self.isForwarding:
                ...
                if 'img' in self.backward_apply_to:
    ...
    def apply_to_seg(self, result):
        for key in result.get('seg_fields', []):
            if self.isForwarding:
                ...
                if key in self.backward_apply_to:
                    slices = tuple([slice(lp, shape - rp) for (lp, rp), shape in zip(self.params, self.image_shape)])
                    slices = (slice(None),) + slices
...
class RandomScale(AugmentationStage):
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += '(scale={})'.format(self.scale)
        return repr_str
    ...
    def _forward_params(self, result):
        super()._forward_params(result)
        scales = (self.get_range(self.scale, 1),) * self.dim
        # 2d (0.5, 0.5)
        # 3d (0.5, 0.5, 0.5)
        self.params = scales
        result[self.key_name] = self.params
...
class RandomRotate(AugmentationStage):
    ...
    def __init__(self, p, rotate, axes=None, reshape=False, order=1, mode='constant', val=0.0):
        super().__init__(p)
        self.rotate = rotate
        self.axes = axes
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += '(p={}, rotate={}, axes={}, order={}, mode={})'.format(
            self.p, self.rotate, self.axes, self.order, self.mode)
        return repr_str
    ...
    def _forward_params(self, result):
        ...
        # 40 deg
        self.params = self.get_range(self.rotate)
        result[self.key_name] = self.params
...
class RandomAffine(AugmentationStage):
    """
    support segmentation, detection and classification tasks
    support 2D and 3D image
    support forward and backward
    ...
    def __init__(self, p, scale, angle, shift=0.1, mode='constant'):
        super().__init__(p)
        self.scale_factor = scale
        self.angle_factor = angle
        self.shift_factor = shift
        self._tmp_params = None

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += '(scale_factor={})'.format(self.scale_factor)
        return repr_str
    ...
    def _forward_params(self, result):
        super()._forward_params(result)
        scales = (self.get_range(self.scale_factor, 1),) * self.dim
        # 2d (0.5, 0.5)
        # 3d (0.5, 0.5, 0.5)
        self.params = scales
        result[self.key_name] = self.params
...
class RandomCrop(OperationStage):
    ...
    def apply_to_img(self, result):
        for key in result.get('img_fields', []):
            pre_shape = result[key].shape
            start, end = self.params
            slices = (slice(None),) + tuple(map(slice, start, end))
            result[key] = result[key][slices]
            if key == 'img':
                result['img_shape'] = result[key].shape
            assert result[key].shape[1:] == self.patch_size, f'{key}: {pre_shape} => {result[key].shape} != {self.patch_size}'
...
class ForegroundCrop(RandomCrop):
    def __init__(self, border=12, foreground='gt_seg', *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.border = border
    ...
    def _forward_params(self, result):
        ...
        assert self.foreground in result.keys(), f'{self.foreground} not in  results.keys()'
        try:
            foreground = result[self.foreground][0]
...
class FirstDetCrop(RandomCrop):
    def __init__(self, patch_size=(128, 128), border=12, **kwargs):
