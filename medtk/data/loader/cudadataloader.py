...
from .to_cuda import _to_cuda_loop
...
class CudaDataLoader(Generic[T_co]):
    r"""
    ...
    def __init__(self, dataset: Dataset[T_co], batch_size: Optional[int] = 1,
                 ...
                 cuda_aug_pipeline: list = None,
                 cuda_collate_fn: _collate_fn_t = None,
        ...
        self.cuda_aug_pipeline = [] if cuda_aug_pipeline is None else cuda_aug_pipeline
        self.cuda_collate_fn = cuda_collate_fn
    ...
    def __setattr__(self, attr, val):
        ...
        super(CudaDataLoader, self).__setattr__(attr, val)
    ...
    def __iter__(self) -> '_BaseDataLoaderIter':
        ...
        if hasattr(self.sampler, 'set_epoch'):
            # print('sampler.set_epoch', self._epoch)
            # change the index order in DistributedSampler
...
class _BaseDataLoaderIter(object):
    def __init__(self, loader: CudaDataLoader) -> None:
        ...
        self._cuda_aug_pipeline = loader.cuda_aug_pipeline
        self._cuda_collate_fn = loader.cuda_collate_fn
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    ...
    def _next_data(self):
        ...
        if len(self._cuda_aug_pipeline):
            for transform in self._cuda_aug_pipeline:
                data = transform(data)
            data = self._cuda_collate_fn(data)
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        self._cuda_num_workers = 0  # 0 for main process (recommended), 1+ for multi processes
        ...
        for i in range(self._num_workers):
            ...
            w = multiprocessing_context.Process(
                ...
                args=(self._dataset_kind, self._dataset, index_queue,
                      ...
                      self._base_seed + i, partial(self._worker_init_fn, epoch=self._epoch),
        ...
        if self._pin_memory:
            ...
            self._mid_data_queue = queue.Queue()  # type: ignore
            pin_memory_thread = threading.Thread(
                target=_utils.pin_memory._pin_memory_loop,
                args=(self._worker_result_queue, self._mid_data_queue,
            ...
            self._mid_data_queue = self._worker_result_queue

        if self._cuda_num_workers > 0 and len(self._cuda_aug_pipeline) > 0:
            self._to_cuda_thread_done_event = self._workers_done_event
            self._data_queue = multiprocessing_context.Queue()
            self._to_cuda_workers = []

            for i in range(self._cuda_num_workers):
                to_cuda_w = multiprocessing_context.Process(
                    target=_to_cuda_loop,
                    args=(self._mid_data_queue, self._data_queue,
                          torch.cuda.current_device(),
                          self._cuda_aug_pipeline,
                          self._cuda_collate_fn,
                          self._to_cuda_thread_done_event))
                to_cuda_w.daemon = True
                to_cuda_w.start()

                self._to_cuda_workers.append(to_cuda_w)
        else:
            self._data_queue = self._mid_data_queue
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _process_data(self, data):
        ...
        if self._cuda_num_workers == 0 and len(self._cuda_aug_pipeline) > 0:
            try:
                if len(self._cuda_aug_pipeline):
                    # the order of data queue is not always the same while workers > 1,
                    # force the seed is dependent on index
                    # optional
                    # random.seed(self._epoch * self._rcvd_idx + 9999)
                    # np.random.seed(self._epoch * self._rcvd_idx + 9999)

                    # print('before', self._rcvd_idx, data)
                    for transform in self._cuda_aug_pipeline:
                        data = transform(data)
                    # print('after', self._rcvd_idx, data)
