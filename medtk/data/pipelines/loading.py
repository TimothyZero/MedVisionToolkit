...
class LoadAnnotations(Stage):
    ...
    def _load_det(result):
        """ [n, (x,y,x,y, cls, score) | (x,y,z,x,y,z, cls, score)] """
        assert isinstance(result['det'], list), 'gt_det should be a list of dict'
...
class PseudoMask(Stage):
    def __repr__(self):
        repr_str = self.__class__.__name__ + '()'
        return repr_str

    def _forward(self, result: dict):
        if result.get('det_fields'):
            dim = result['img_dim']
            pseudo_mask = np.zeros_like(result['img'][[0], ...])
            for ann in result['gt_det']:
                # (x1,y1,x2,y2, class, score) | (x1,y1,z1,x2,y2,z2, class, score)
                # draw pseudo mask
                slices = list(map(slice, reversed(np.int32(ann[:dim])), reversed(np.int32(ann[dim:2*dim]))))
                slices = [slice(None)] + slices
                pseudo_mask[tuple(slices)] = ann[2*dim]
